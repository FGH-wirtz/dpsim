...
void EMT_IEEE_39Bus_3ph(Real timeStep, Real finalTime) {
  ...
  filenames = Utils::findFiles(
      {"Rootnet_FULL_NE_29J10h_SV.xml", "Rootnet_FULL_NE_29J10h_EQ.xml",
  ...
  r16_17->setParameters(
      CPS::Math::singlePhaseParameterToThreePhase(1));
...
void EMT_IEEE_39Bus_3ph_Diakoptics_openMP(Real timeStep, Real finalTime) {
  ...
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> elapsed = end - start;
  std::cout << "IEEE-39 diakoptics_openMP: Zeit: " << std::fixed << std::setprecision(2)
            << elapsed.count() << " Sekunden\n";
}
